# https://github.com/thomashikaru/gogame
...
class Game:
    ...
    def init_pygame(self):
        ...
        self.screen = screen
        self.ZOINK = pygame.mixer.Sound("/Users/sarakim/venv-ml/go_env/wav/zoink.wav")
        self.CLICK = pygame.mixer.Sound("/Users/sarakim/venv-ml/go_env/wav/click.wav")
    ...
    def handle_click_test(self):
        # get board position
        x, y = pygame.mouse.get_pos()
        #colrow_to_xy(col, row, self.size)
        col, row = xy_to_colrow(x, y, self.size)
        print(col)
        print(row)
        if not is_valid_move(col, row, self.board):
    ...
    def update(self,pass_twice):
        ...
        for event in events:
            ...
            if event.type == pygame.QUIT:
                if (int(self.prisoners['black']) > int(self.prisoners['white'])):
                    txt = self.font.render("Black Wins", True, BLACK)
                    self.screen.blit(txt, SCORE_POS)
                elif (int(self.prisoners['black']) < int(self.prisoners['white'])):
                    txt = self.font.render("White Wins", True, BLACK)
                    self.screen.blit(txt, SCORE_POS)
                time.sleep(20)
                sys.exit()
...
if __name__ == "__main__":
    ...
    while True:
        print(pass_twice)
        pass_twice = g.update(pass_twice)
